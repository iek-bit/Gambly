...
def _remove_active_multiplayer_presence():
    current_account = st.session_state.get("current_account")
    for player_name in (current_account,):
        if not player_name:
            continue
        try:
...
def _auto_remove_poker_lan_player_when_not_in_poker():
    current_account = st.session_state.get("current_account")
    active_player = current_account
    if not active_player:
        return
    if st.session_state.get("active_action") == "Poker" and not st.session_state.get("show_auth_flow", False):
        return
...
def main():
    ...
    if st.session_state.get("active_action") not in allowed_actions:
        st.session_state["active_action"] = "Home"
    _auto_remove_poker_lan_player_when_not_in_poker()
